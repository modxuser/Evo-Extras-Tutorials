...
6. Remove the code from point 5 above and replace it with the following Doclister call:
   ...
   !]
   <p class="center">Viewing <strong>[+list.from+]</strong> - <strong>[+list.to+]</strong> of <strong>[+list.count+]</strong> for <em>[*pagetitle*]</em> | Page <strong>[+list.current+]</strong> of <strong>[+list.totalPages+]</strong></p>
   [+list.pages+]
   ```
   
   All of the classes are from Bootstrap.
