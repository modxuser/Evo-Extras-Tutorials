...
3. [Section 3: Display Category Tags In Blog Overview + Blog Posts](#section-3-display-category-tags-in-blog-overview--blog-posts)
    - [Add Category Tags In Blog Overview](#add-category-tags-in-blog-overview)
    - [Add Category Tags In Blog Posts](#add-category-tags-in-blog-posts)
4. [Section 4: Filter Content In Tag Landing Pages]()
...
# Section 3: Display Category Tags In Blog Overview + Blog Posts

Here we need to make modifications to the template(s) and chunks that have previously be created.

<br>

### Add Category Tags In Blog Overview
...
```
[[tagLinks? &id=`[+id+]` &path=`2` &tv=`myTags` &value=`[+myTags+]` &label=`Tags: ` &separator=`, ` &format=`a` &element=`div` &newline=`1`]]
```

Your chunk should look something like this:
...
![blogoverviewwithtags](https://user-images.githubusercontent.com/1859868/113868646-325e2100-97b0-11eb-91ca-bdf92d0ff813.png)

<br>

### Add Category Tags In Blog Posts

As we created the [Blog Page Template](#create-a-blog-page-template) we added the following code:
